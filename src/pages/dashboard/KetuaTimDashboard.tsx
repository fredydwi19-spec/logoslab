...
export const KetuaTimDashboard = () => {
  ...
  useEffect(() => {
    if (!kpiLoading && !kpiError && chartRef.current) {
      if (chartInstance.current) {
        chartInstance.current.destroy();
      }
      const labels = spiderData.labels || ['Content Velocity','Expert Responsiveness','User Engagement','Passing Rate','Category Coverage'];
      const data = spiderData.data || [0,0,0,0,0];
  ...
  }, [spiderData, kpiLoading, kpiError]);
  ...
  if (kpiError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
        ...
        <button onClick={fetchData} className="mt-4 bg-[#1A237E] text-white text-xs font-black uppercase tracking-widest px-6 py-2 rounded-lg hover:bg-blue-900 transition-colors">Coba Lagi</button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100">
          ...
          <div className="space-y-3">
            {funnel.length > 0 ? funnel.map((stage, idx) => (
              <div key={idx} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-slate-100 rounded-full h-8 overflow-hidden">
                  <div className="h-full rounded-full flex items-center justify-end pr-3 transition-all"
      ...
      <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100">
        ...
        <div className="flex items-end gap-2">
          {heatmap.length > 0 ? heatmap.map((day, idx) => (
